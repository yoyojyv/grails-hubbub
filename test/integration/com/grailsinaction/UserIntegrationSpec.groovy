...
class UserIntegrationSpec extends IntegrationSpec {
  ...
  def "Updating a saved user change its properties"() {
  ...
  def "Saving a user with invalid properties cause an error"() {
...
//    "not-a-url" == user.errors.getFieldError("homepage").rejectedValue
  ...
  def "Recovering from a failed save by fixing invalid properties"() {
    ...
    then: "The user saves and validate fine"
    !chuck.hasErrors()
    chuck.save()
  ...
  def "Ensure a user can follow other users"() {
    ...
    given: "A set of follow other users"
